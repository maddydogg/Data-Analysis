#!/usr/bin/env bash
# Build the 14.9 s Etsy listing video for the DARK Budget Planner workbook.
#   usage: build_budget_video_dark.sh <raw-recording> <work-dir> <output.mp4>
#
...
python3 "$HERE/make_layers_budget_dark.py" "$W/layers" "$W/fonts"
...
# edited, then carries on from 16.0 s to 18.2 s. Both stretches are one pass
# down the same page; the rows the edit touched are all below 641 px, so they
# come from the later stretch and the page is internally consistent.
PASS="3.4,3.6,3.8,4.0,4.4,4.8,5.0,5.2,5.4,5.6,6.0,6.2,6.6,6.8,7.0,7.2,7.6,7.8,8.0,8.2,8.4,8.8,9.2,16.0,16.2,16.4,16.6,16.8,17.0,17.4,17.6,18.0,18.2"
python3 "$HERE/make_page.py" "$W/src/rec.mp4" "$W/scenes/s01.mp4" "$PASS" 1470

# 2. the four cuts that follow. Sheets never raised its "convert to table"
#    toast during this take, so nothing has to be steered around; the frames
#    were checked for it anyway. The month pair and the wide Log are framed at
#    the page's full width, the same scale as the scroll, so the cut never
#    changes the zoom — the point being that this is all still one page.
SHOTS=(
  "25.20 1.40  1440:810:0:0    month-list-open"
  "26.70 1.60  1440:810:0:0    december-applied"
  "34.30 1.55  1440:810:0:0    log"
  "48.20 1.74  900:506:0:300   log-category-list"
)
i=1
for s in "${SHOTS[@]}"; do
  set -- $s; i=$((i+1))
...
CAPS=("0.15 3.10" "3.45 6.30" "6.65 9.20" "9.60 10.60" "10.85 12.05" "12.30 14.62")
PILLS=("0.00 12.00" "12.00 14.90")
