#!/usr/bin/env bash
# Build the 14.9 s Etsy listing video for the Bookkeeping workbook.
#   usage: build_books_video.sh <raw-recording> <work-dir> <output.mp4>
#
...
python3 "$HERE/make_layers_books.py" "$W/layers" "$W/fonts"

# 1. shot 1 — the Books sheet, held and then scrolled, in one take. The take
#    scrolls it down between 0.1 s and 12.6 s and then back up; these are the
#    downward pass's resting moments, so the stitch takes no post-scroll ghost.
PASS="0.1,0.6,0.9,1.2,1.5,1.9,2.2,2.5,2.8,3.1,3.4,3.6,4.0,4.2,4.5,4.8,5.2,5.4,5.7,6.0,6.3,6.6,6.9,7.2,7.5,7.8,8.1,8.4,8.7,9.0,9.3,9.6,9.9,10.2,10.5,10.8,11.1,11.4,11.7,12.0,12.2,12.6"
python3 "$HERE/make_page.py" "$W/src/rec.mp4" "$W/scenes/s01.mp4" "$PASS" 1360 "1.80,6.10,0.70"
...
SHOTS=(
  "21.80 2.45  1360:765:0:48   period-switch"
  "32.90 2.10  1360:765:0:50   log"
  "34.20 2.59  800:450:0:60    type-list"
)
i=1
for s in "${SHOTS[@]}"; do
  set -- $s; i=$((i+1))
...
CAPS=("0.15 2.70" "3.00 5.40" "5.70 8.10" "8.50 9.55" "9.75 10.75" "10.85 12.10" "12.55 14.62")
PILLS=("0.00 10.55" "10.55 14.90")
...
FC2+="[$PREV]fade=t=in:st=0:d=0.35:color=0xFAF9F4,fade=t=out:st=14.6:d=0.3:color=0xFAF9F4,format=yuv420p[out]"
