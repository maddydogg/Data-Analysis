"""Design layers for the dark ADHD Planner video (1080x1080).

Same ground as the other dark listings — deep navy, a green glow behind the
window so the dark sheet sits in light, the accent the #7BFFA2 this workbook
writes its own section headers in. Three tab pills, and the captions are the
light cut's: lifted from the sheet, one per beat, because the competitor this
listing answers runs one headline for fifteen seconds and explains nothing.
"""
...
BG_HI   = (40, 30, 64)
BG_LO   = (13, 10, 26)
ACCENT  = (123, 255, 162)      # 7BFFA2 — straight off the dark sheet
TEXT    = (234, 243, 240)
MUTED   = (154, 146, 186)
WIN_BAR = (36, 30, 60)
WIN_BODY= (14, 17, 35)         # 0E1123 — the sheet's own ground
EDGE    = (62, 52, 96)
PILL_TX = (160, 150, 190)
...
def tab_layout(d):
    """Two tabs — the product is two sheets, so the strip says so."""
    fnt = M600(13)
    tr = 1.1
...
bg.paste(Image.new("RGB", (S, S), (150, 110, 210)), (0, 0), halo.filter(ImageFilter.GaussianBlur(110)))

draw_tracked(d, S / 2, 44, "TWO SHEETS  ·  SHELF  ·  BOOKS  ·  GOOGLE SHEETS & EXCEL", M600(16), MUTED, tr=3.4)
draw_tracked(d, S / 2, 76, "Reading Tracker", PF700(62), TEXT, tr=0.5)
...
for cx in (WIN[0] + 26, WIN[0] + 46, WIN[0] + 66):
    d.ellipse([cx - 5, WIN[1] + 20, cx + 5, WIN[1] + 30], fill=(80, 68, 120))
d.rounded_rectangle([WIN[0] + 92, WIN[1] + 14, WIN[0] + 520, WIN[1] + 36], radius=11, fill=(24, 19, 44))
d.text((WIN[0] + 108, WIN[1] + 25), "reading-tracker-2026", font=M500(13), fill=(132, 120, 168), anchor="lm")
chip = "LOG A BOOK ON BOOKS"
...
draw_tracked(d, WIN[2] - 20 - chip_w / 2, WIN[1] + 18, chip, M700(12), (8, 20, 14), tr=1.6)
...
d.rectangle([SCREEN[0] - 1, SCREEN[1] - 1, SCREEN[0] + SCREEN[2] + 1, SCREEN[1] + SCREEN[3] + 1],
            outline=(56, 46, 88), width=1)

fnt_tab, boxes, TR = tab_layout(d)
for (x, w), label in zip(boxes, TABS):
    d.rounded_rectangle([x, TAB_Y, x + w, TAB_Y + TAB_H], radius=TAB_H / 2,
                        fill=(28, 23, 50), outline=EDGE, width=1)
    draw_tracked(d, x + w / 2, TAB_Y + 8, label.upper(), fnt_tab, PILL_TX, tr=TR)

draw_tracked(d, S / 2, 950, "LOG A BOOK ONCE — THE SHELF COUNTS, RATES AND PACES IT FOR YOU", M600(15), TEXT, tr=1.4)
draw_tracked(d, S / 2, 980, "by month · by genre · by format · ratings · reading now · up next", M400(14), MUTED, tr=0.6)
d.rectangle([0, S - 5, S, S], fill=(40, 32, 66))
...
# ---------------------------------------------------------------- captions
# The sheet's own words, timed against the film rather than against shots.
CAPTIONS = [
    "Two sheets. Log a book on Books.",
    "Books finished, by month",
    "By genre, by format, by rating",
    "Pick a period…",
    "…and the shelf re-counts",
    "One row per book",
    "Reading, finished, did not finish",
]
for i, caption in enumerate(CAPTIONS, 1):
    lay = Image.new("RGBA", (S, S), (0, 0, 0, 0))
    cd = ImageDraw.Draw(lay)
    size = 30 if len(caption) < 30 else (27 if len(caption) < 40 else 24)
    draw_tracked(cd, S / 2, 180, caption, M500(size), TEXT + (255,), tr=0.3)
...
for i, tab in enumerate(TABS, 1):
    ...
    draw_tracked(pd, x + w / 2, TAB_Y + 9, tab.upper(), fnt_tab, (8, 20, 14, 255), tr=TR)
