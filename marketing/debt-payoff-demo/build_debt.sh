#!/usr/bin/env bash
# Build the 14.9 s Etsy listing video for the Debt Payoff workbook (paper frame).
...
if [ ! -f "$W/src/rec.mp4" ]; then
  "$FF" -y -hide_banner -loglevel error -fflags +genpts -i "$RAW" \
    -vf "crop=1622:720:0:0" -r 30 -c:v libx264 -preset veryfast -crf 18 -pix_fmt yuv420p "$W/src/rec.mp4"
fi

python3 "$HERE/make_layers_debt.py" "$W/layers" "$W/fonts"

SW=960; SH=540; D=1.742; X=0.28
...
# 1. ten shots. This recording is 1622x720 with no row-number gutter, so crops
#    start at x=0. The take is clean — no Sheets toast, no tab-switch flashes.
cut s01 0.4  "crop=820:461:0:6"      # Setup — the six debts
cut s02 2.6  "crop=820:461:0:6"      # Setup — Method dropdown open
cut s03 13.0 "crop=900:506:0:6"      # Dashboard — debt-free date, months, debt, interest
cut s04 21.4 "crop=760:428:0:150"    # Dashboard — total paid, paid so far, progress
cut s05 15.4 "crop=900:506:0:150"    # Dashboard — payoff order
cut s06 17.4 "crop=924:520:0:150"    # Dashboard — balance over time
cut s07 19.2 "crop=860:484:0:200"    # Dashboard — Snowball vs Avalanche + the verdict
cut s08 29.8 "crop=820:461:0:6"      # Setup — switching the method to Custom
cut s09 32.4 "crop=900:506:0:6"      # Dashboard — Custom: Jul 2029, 35 months, $8,662
cut s10 25.4 "crop=820:461:0:6"      # Debt Tracker — paid so far, remaining, % paid
...
for i in $(seq 1 10); do
  ...
  CIN=$(python3 -c "print(round($ST+0.10,3))"); COUT=$(python3 -c "print(round($ST+1.26,3))")
  IDX=$((3+i))
