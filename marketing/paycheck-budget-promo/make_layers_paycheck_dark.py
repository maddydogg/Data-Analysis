"""Design layers for the dark Paycheck Budget video (1080x1080).

Palette sampled from the recording itself: sheet background #0E1123, panels
#151D2F, the accent green #7BFFA2 the dark workbook writes its headers in, white
type. The frame sits a shade lighter and bluer than the sheet so the screen
reads as a window on a page. Same geometry as the light cut — only the ground,
the glow and the type colour change.
"""
...
def tab_layout(d):
    """Eight tabs — two more than the debt listing carries, so the strip is
...
draw_tracked(d, S / 2, 44, "PAYCHECK 1  ·  PAYCHECK 2  ·  GOOGLE SHEETS & EXCEL", M600(16), MUTED, tr=3.4)
draw_tracked(d, S / 2, 76, "Paycheck Budget", PF700(62), TEXT, tr=0.5)
...
d.text((WIN[0] + 108, WIN[1] + 25), "paycheck-budget-2026", font=M500(13), fill=(104, 124, 160), anchor="lm")
chip = "DOES THE MATH"
...
draw_tracked(d, S / 2, 950, "TWO PAYCHECKS, TWELVE CATEGORIES — THE MATH IS ALREADY DONE", M600(15), TEXT, tr=1.7)
draw_tracked(d, S / 2, 980, "8 tabs · budget by paycheck, not by month · yours to edit", M400(14), MUTED, tr=0.6)
...
SCENES = [
    ...
    ("Weekly and yearly bills, as a monthly cost",    "Bill Calendar"),
    ("Change one frequency…",                         "Bill Calendar"),
    ("…and the monthly total re-does itself",         "Bill Calendar"),
...
for i, (caption, tab) in enumerate(SCENES, 1):
    ...
    draw_tracked(pd, x + w / 2, TAB_Y + 8, tab.upper(), fnt_tab, (8, 20, 14, 255), tr=TR)
